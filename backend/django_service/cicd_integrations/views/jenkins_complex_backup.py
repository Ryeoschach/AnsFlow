...
class JenkinsManagementMixin:
    ...
    def jenkins_list_jobs(self, request, pk=None):
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                return Response(
                    {'error': f'Jenkins API returned status {response.status_code}'},
    ...
    def jenkins_job_info(self, request, pk=None):
        ...
        try:
            ...
            return Response({
                ...
                'job_info': {
                    'name': job_name,
                    'url': f"{tool.base_url}/job/{job_name}",
                    'buildable': True,
                    'lastBuild': None
                }
            })
    ...
    def jenkins_create_job(self, request, pk=None):
        """创建Jenkins作业"""
        tool = self.get_object()
        ...
        try:
            ...
            return Response({
                'tool_id': tool.id,
                'job_name': job_name,
                'message': f'Job "{job_name}" creation initiated (simplified implementation)',
    ...
    def jenkins_delete_job(self, request, pk=None):
        ...
        try:
            ...
            return Response({
                'tool_id': tool.id,
                'job_name': job_name,
                'message': f'Job "{job_name}" deletion initiated (simplified implementation)'
            })
    ...
    def jenkins_start_build(self, request, pk=None):
        """启动Jenkins构建"""
        tool = self.get_object()
        ...
        try:
            ...
            return Response({
                'tool_id': tool.id,
                'job_name': job_name,
                'message': f'Build for job "{job_name}" started (simplified implementation)',
                'build_queue_item': 123
    ...
    def jenkins_stop_build(self, request, pk=None):
        """停止Jenkins构建"""
        tool = self.get_object()
        ...
        if not job_name or not build_number:
            return Response(
                {'error': 'Both job_name and build_number are required'},
                status=status.HTTP_400_BAD_REQUEST
            )
        ...
        try:
            ...
            return Response({
                ...
                'message': f'Build #{build_number} for job "{job_name}" stopped (simplified implementation)'
            })
    ...
    def jenkins_build_logs(self, request, pk=None):
        ...
        return Response({
            'tool_id': tool.id,
            'logs': "Build log placeholder (simplified implementation)",
            'message': 'Simplified implementation'
        })

    @action(detail=True, methods=['get'], url_path='jenkins/queue-info')
    def jenkins_queue_info(self, request, pk=None):
        """获取构建队列信息"""
        tool = self.get_object()
        validation_error = self._validate_jenkins_tool(tool)
        if validation_error:
    ...
    def jenkins_enable_job(self, request, pk=None):
        """启用Jenkins作业"""
        tool = self.get_object()
        validation_error = self._validate_jenkins_tool(tool)
        if validation_error:
        ...
        return Response({
            'tool_id': tool.id,
            'message': 'Job enabled (simplified implementation)'
        })

    @action(detail=True, methods=['post'], url_path='jenkins/disable-job')
    def jenkins_disable_job(self, request, pk=None):
        """禁用Jenkins作业"""
        tool = self.get_object()
        validation_error = self._validate_jenkins_tool(tool)
        if validation_error:
        ...
        return Response({
            'tool_id': tool.id,
            'message': 'Job disabled (simplified implementation)'
        })
    ...
    def jenkins_job_info(self, request, pk=None):
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                return Response(
                    {'error': f'Failed to get job info: {response.status_code}'},
                    status=status.HTTP_500_INTERNAL_SERVER_ERROR
                )
...
pipeline {
  ...
  <disabled>false</disabled>
            ...
            try:
                ...
                pass  # 如果获取 crumb 失败，继续尝试不使用
            
            # 创建作业
            response = requests.post(
                f"{tool.base_url}/createItem",
            ...
            if response.status_code in [200, 201]:
                ...
                return Response(
                    {'error': f'Failed to create job "{job_name}": {response.status_code}'},
                    status=status.HTTP_500_INTERNAL_SERVER_ERROR
                )
    ...
    @action(detail=True, methods=['delete'], url_path='jenkins/delete-job')
    def jenkins_delete_job(self, request, pk=None):
        """删除Jenkins作业 - 暂时禁用异步功能"""
        return Response(
            {'error': 'This feature is temporarily disabled. Please use Jenkins web interface.'},
